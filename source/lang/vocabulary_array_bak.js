...
class ArrayVocabularyImplementation extends Vocabulary {
    ...
    constructor(forth, options = {}) {
        ...
        this.add_word('[', this.array_brace_word, '');
    ...
    array_brace_word({interpreter}) {
        const context = interpreter.forth.new_context();
        const [, items] = interpreter.forth.compile_until([']'], context);
        ...
        const id = interpreter.forth.objects.create('array', array);

        // now we "run" the array items to let them put items on the sta

        return ({interpreter}) => {
            const forth = interpreter.forth;
